#!/usr/bin/env python3
"""Create a GitHub release when package.json version changes on push; body from CHANGELOG.md."""
...
def changelog_fallback_link(tag: str) -> str:
    ...
    return f"See [CHANGELOG.md]({url}) for details."


def write_output(name: str, value: str) -> None:
    """Append a GitHub Actions output when ``GITHUB_OUTPUT`` is set."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Path to package.json relative to repo root",
    )
    parser.add_argument(
        "--changelog-path",
    ...
    package_file = repo_root / args.package_json_path
    changelog_file = repo_root / args.changelog_path

    current = read_version(package_file.read_text(encoding="utf-8"))
    if not current:
        print("Could not parse version from current package.json; skipping.", file=sys.stderr)
        write_output("created", "false")
        return 0

    parent_text = git_show_parent_file(repo_root, args.package_json_path)
    if parent_text is None:
        print("No parent commit for package.json; skipping.", file=sys.stderr)
        write_output("created", "false")
        return 0
    previous = read_version(parent_text)
    if not previous:
        print("Could not parse version in parent; skipping.", file=sys.stderr)
        write_output("created", "false")
        return 0
    ...
    # Version changed: publish even when the tag already exists so a failed
    # npm upload can be retried with "Re-run all jobs".
    write_output("created", "true")
    tag = f"v{current}"
    if tag_exists_on_remote(repo_root, tag):
        print(f"Tag {tag} already exists on origin; skipping create.")
        return 0
